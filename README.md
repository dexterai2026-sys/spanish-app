...
Everything lives in one index.html: 368 chunks, 802 example sentences, 16 dialogues, 31 sentence patterns, a spaced-repetition engine, a pronunciation scorer and eight study modes. No build step, no dependencies, no backend, no network calls.
...
So the fillers are scoped to the pattern they belong to. Every option in a slot is written to read naturally after that particular lead, so every sentence the app can build is one someone would actually say. 31 patterns, about 210 sentences, all of them real — fewer than free combination would give and worth having.
