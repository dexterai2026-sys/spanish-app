...
So the fillers are scoped to the pattern they belong to. Every option in a slot is written to read naturally after that particular lead, so every sentence the app can build is one someone would actually say. 52 patterns, all of them real — fewer than free combination would give and worth having.
...
Built from your own words. Eleven patterns fill their slot from vocabulary you have actually studied rather than from a fixed list, preferring words that are due — so building a sentence is also a review, and a studied word becomes a used one. Playing the first week showed why this was needed: of 84 words met, only 5 had ever appeared in a sentence the learner could build. Words were being remembered and never becoming usable, which is the met-once problem wearing a new costume.

423 of the 564 words are reachable this way, across ten domains. The rest sit in slots no frame safely takes.

Each such slot declares which domains are safe in it and which surface the frame needs, because "me gusta el café", "tengo un hermano" and "estoy cansado" are three different shapes:

Guard	Stops
Slots name their domains and surface	necesito el dinero
State adjectives only after estoy	estoy grosero
Trait adjectives only after es muy	es muy cansado
Object-taking verbs excluded from bare frames	quiero dar
Plurals excluded from indefinite frames	tengo unos papás
...
564 everyday words are now first-class material, scheduled through the same SM-2 engine as chunks. Half of reviews ask you to recall and grade yourself; the other half show the English against three real alternatives from the same domain and grade objectively, because self-grading alone lets "I sort of knew that" pass as recall — which is how a deck stays green while the learner stays unable to produce anything. Nouns carry their article, because la mano is how you learn the gender and "mano, feminine" is how you forget it.

Words you have studied are also preferred inside the example sentences shown in Learn, so vocabulary keeps resurfacing in context rather than being met once and filed away.
